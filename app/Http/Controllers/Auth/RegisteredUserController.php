...
class RegisteredUserController extends Controller
{
    ...
    public function storeAdmin(Request $request): RedirectResponse
    {
        ...
        Auth::login($user);
        if ($user->role == 1) {
            return redirect()->route('user.listUser')->with('success', 'Admin ajouté avec succès!');
        } else {
            return redirect()->route('reservation.create')->with('success', 'Client ajouté avec succès!');
        }
    }
}
